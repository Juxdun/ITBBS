...
<body>
	...
	<div class="viewuser-div">
			...
			<div id="commentTo" style="float:left">
					<span style="color:red;">您所在的位置：我的评论</span>
					<c:forEach items="${mycomment}" var="cr">
					<!-- 第一行：您 回复谁在 XX话题 发表的评论；您评了谁的话题 -->
					<c:if test="${cr.type==1}">
						<div class="head_info" style="margin:10px;">	
								<span >您对</span>
								<span ><a href="/viewuser.jspx?id=${cr.comment.topic.user.id}" style="color:#00B7E9">
									<c:choose>
										<c:when test="${cr.comment.topic.user.nickname!=null}">
											${cr.comment.topic.user.nickname}
										</c:when>
										<c:otherwise>
											${cr.comment.topic.user.username}
										</c:otherwise>
									</c:choose>
								</a></span>
								<span>发表话题</span>
						...
						<div class="footer_info" style="margin:10px;" >
							...
							<div class="bigmore"><a href="/user/editcomment.jspx?type=comment&id=${cr.comment.id}" class="node_read_more">修改评论</a></div>
						</div>
					</c:if>
					<c:if test="${cr.type==2}">
						<div class="head_info" style="margin:10px;">
								...
								<span ><a href="/viewuser.jspx?id=${cr.reply.comment.user.id}" style="color:#00B7E9">
									<c:choose>
										<c:when test="${cr.reply.comment.user.nickname!=null}">
											${cr.reply.comment.user.nickname}
										</c:when>
										<c:otherwise>
											${cr.reply.comment.user.username}
										</c:otherwise>
									</c:choose>
								</a></span>
								<span>在话题</span>
						...
						<div class="footer_info" style="margin:10px;" >
							...
							<div class="bigmore"><a href="/user/editcomment.jspx?type=reply&id=${cr.reply.id}" class="node_read_more">修改回复</a></div>
						</div>
					</c:if>	
					
					</c:forEach>
					 <c:if test="${mycomment.pageCount>0}">
        				<div id="page" class="pager"></div>
       				 </c:if>  
			</div>
			<c:choose>
		 		<c:when test="${mycomment.pageCount==0}">
		 			<input id="pagecount" type="hidden" value="1"/>
		 		</c:when>
		 		<c:otherwise>
		 			<input id="pagecount" type="hidden" value="${mycomment.pageCount}"/>
		 		</c:otherwise>
		 	</c:choose>
...
<script type="text/javascript">
  ...
  var pageClick = function() {
    ...
    oAlink[0].onclick = function() { //点击上一页
      ...
      inx--;
      window.location.href='/user/mycomment.jspx?p='+inx;
      return false;
    }
