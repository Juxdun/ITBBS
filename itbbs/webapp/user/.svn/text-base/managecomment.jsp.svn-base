...
<jsp:include page="../menu.jsp" />
	...
	<div class="container">
        ...
        <span style="color:red;margin-left:46px">您所在的位置：评论管理</span>
		<div class="former">
			<h3 class="title" style="margin-bottom:10px;">搜索评论</h3>
		...
		<div class="former">
			...
			<table class="tabler">
				...
				<tbody>
					...
					<tr>
						<td class="td-height"><a href="/viewuser.jspx?id=${c.user.id}">${c.user.username}</a></td>
						<td class="td-height"><p>${c.content}</p></td>
						<td class="td-height"><a href="/viewforum.jspx?id=${c.topic.id}">${c.topic.name}</a></td>
						<td class="td-height"><fmt:formatDate value="${c.date}" pattern='yyyy-MM-dd hh:mm:ss' /></td>
						<td class="td-height">${c.status.name}</td>
						<td class="td-height">
							<c:if test="${c.status.id==1}">
							<a class="button a-margin" href="?op=shield&id=${c.id}&username=${username}&topicId=${topicId}&p=${p}">屏蔽</a>
							
							</c:if>
							<c:if test="${c.status.id==2}">
							<a class="button a-margin" href="?op=recover&id=${c.id}&username=${username}&topicId=${topicId}&p=${p}">恢复</a>
